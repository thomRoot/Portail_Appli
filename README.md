...
4. **Configurer dans `app.js`** :
   Remplacez dans le fichier `public/app.js` :
   ```javascript
   const ROBOT_CONFIG = {
...
Dans le fichier `public/app.js`, remplacez :
```javascript
...
Modifiez les coordonnées dans `public/app.js` :
```javascript
...
Modifiez la configuration dans `public/app.js` pour chaque robot :
...
### **V1.3e - Midnight Blue (Version précédente)**
- **Style complet** : Thème "Midnight Blue" avec dégradés, étoiles animées et effets de glow
- **Uniformisation des tailles** : Les éléments `headerCurrentTemp` (température dans le header) ont été alignés sur une taille de **100px × 100px** pour une cohérence visuelle.
...
2. Configurez l'adresse IP et le token dans `public/app.js`
...
**Causes possibles** :
1. **Clé API non configurée** → Vérifiez que vous avez remplacé `votre_cle_api_ici` dans `public/app.js`
...
*Style : V2.0_pas_de_rer_B - Midnight Blue*
